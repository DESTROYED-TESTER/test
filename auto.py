...
import re
from datetime import datetime
from faker import Faker  # To generate a fake name

fake = Faker()

# Function to generate a random temporary email
def get_temp_plus():
    name = " ".join(fake.name()).replace(' ', '')  # Generate a fake name without spaces
    jam = str(datetime.now().strftime("%X")).replace(':', '')  # Remove colons from the current time
...
# Function to get the code from the temp email
def get_code_temp_plus(email):
    ...
    try:
        # Sending GET request to fetch the emails
        ...
        if response.status_code == 200:
            ...
            # If mail list is not empty
            if mail_list:
                subject = mail_list[0].get('subject', '')
                kode = re.search(r'(\d+)', subject)  # Search for numeric code in the subject
                
                if kode:
                    return kode.group(1)  # Return the found code
                else:
                    return 'Code not found in the subject.'
            else:
                return 'No emails found.'
        else:
            return f'Failed to retrieve emails, status code: {response.status_code}'
