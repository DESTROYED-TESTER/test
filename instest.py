...
class InstagramLogin:
    def __init__(self, username, passwd, useragent):
        self.username = '8918354921'
        self.passwd = '891835'
    ...
    def Simpan_Result(self):
        # Placeholder for the actual implementation of Simpan_Result
        return None, None, None

    def login(self):
        try:
            self.hash = hashlib.md5()
            self.hash.update(self.username.encode('utf-8') + self.passwd.encode('utf-8'))
            ...
            payload = {
                'params': '{"client_input_params":{"device_id":"'+ str(headers['x-ig-android-id']) +'","lois_settings":{"lois_token":"","lara_override":""},"name":"'+str(self.username)+'","machine_id":"'+str(headers['x-mid'])+'","profile_pic_url":null,"contact_point":"'+str(self.username)+'","encrypted_password":"#PWD_INSTAGRAM:0:'+str(int(time.time()))+':'+str(self.passwd)+'"},"server_params":{"is_from_logged_out":0,"layered_homepage_experiment_group":null,"INTERNAL__latency_qpl_marker_id":36707139,"family_device_id":"'+str(headers['x-ig-family-device-id'])+'","device_id":"'+str(headers['x-ig-device-id'])+'","offline_experiment_group":null,"INTERNAL_INFRA_THEME":"harm_f","waterfall_id":"'+str(uuid.uuid4())+'","login_source":"Login","INTERNAL__latency_qpl_instance_id":73767726200338,"is_from_logged_in_switcher":0,"is_platform_login":0}}',
